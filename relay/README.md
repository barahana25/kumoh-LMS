...
1. **DDNS**: 제어판 → 외부 액세스 → DDNS → 추가. 서비스 공급자 Synology, 호스트 이름 `barahana.synology.me`
   - 이 NAS는 443 포트를 다른 서비스가 쓰고 있어 중계 서버를 **8443**으로 연다. 포트를 바꾸면 5의 소스 포트, 6의 공유기 포워딩, 7·8의 주소를 함께 바꾼다
...
5. **리버스 프록시**: 제어판 → 로그인 포털 → 고급 → 리버스 프록시 → 생성
   - 소스: HTTPS, 호스트 이름 `barahana.synology.me`, 포트 8443
   ...
   - 인증서: 제어판 → 보안 → 인증서 → 설정에서 `barahana.synology.me:8443` 항목에 2의 인증서를 지정한다
     - 지정하지 않으면 Synology 기본 자체 서명 인증서(`CN=synology`)가 쓰인다. 브라우저는 WebSocket 인증서 경고를 띄우지 않고 연결만 끊어 앱에는 "네트워크에 연결할 수 없습니다"만 보인다. `curl.exe https://barahana.synology.me:8443/healthz`가 인증서 오류 없이 `ok`인지 확인한다
...
6. **공유기**: TCP 8443 → NAS만 포워딩한다. Let's Encrypt 발급·갱신에는 TCP 80 → NAS도 필요하다. DSM 관리 포트 5000/5001은 외부에 열지 않는다
7. **확인**: `https://barahana.synology.me:8443/healthz`가 `ok`
8. GitHub 저장소 → Settings → Secrets and variables → Actions → Variables에 `RELAY_URL` = `wss://barahana.synology.me:8443/` 등록 (끝의 `/` 필수)
