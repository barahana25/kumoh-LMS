...
공지사항 탭에서 전체·공지사항·파일·토론·과제를 선택해 새 소식 내역을 확인한다. 시스템 알림 전송 후에도 내역은 남으며, 같은 계정에서 알림을 껐다 켜도 유지된다. 로그아웃하면 삭제한다. 목록의 알림 시각은 감지 시각이다. 이미 구버전에서 삭제된 전송 내역은 복원하지 못한다. 토론은 새 게시글만 감지하고 댓글은 포함하지 않는다. 첫 조회는 기존과 같이 기준만 저장한다.

## 토글과 수동 확인

알림 토글과 자동 다운로드 토글은 설정을 켜고 예약만 건다. 토글을 누른 자리에서 학교 서버를 조회하지 않는다. 즉시 확인하려면 `지금 확인`, 즉시 저장하려면 `지금 다운로드`를 누른다. 알림의 첫 조회는 기존과 같이 기준만 저장하고 알리지 않는다.
...
- 대상은 공개되고 잠기지 않은, 마감이 있는 미제출 과제다. 제출 방식이 "없음"·"지필"·"채점 안 함"뿐인 과제는 뺀다. 교수가 면제한 과제도 뺀다.
- 알림을 누르면 해당 강좌의 과제 탭이 열린다. Android 알림 채널은 "과제 마감"(`lms_due`)이다.
- 새 소식과 마감 알림은 매시 작업에서 로그인 한 번과 과제 목록 응답(`include[]=submission`)을 나눠 쓴다. 두 기능의 잠금을 네트워크 전에 한꺼번에 잡아서, 같은 회차에 작업이 겹쳐 들어와도 대개 로그인이 늘지 않는다. 두 작업이 수 밀리초 안에 동시에 시작되는 드문 경우에만 그 회차에 한 번 더 로그인할 수 있다. 자동 다운로드는 지금처럼 따로 로그인한다. LINUS는 계정당 가장 최근 로그인 하나만 유효해 로그인이 늘면 다른 기기 세션이 더 자주 끊기기 때문이다.
- 토글을 켠 그 회차에는 확인하지 않고 다음 회차부터 확인한다. 켜자마자 로그인이 한 번 더 일어나지 않게 하기 위해서다.
...
| `data/shared_alerts.dart` | 두 러너의 잠금을 먼저 잡고 로그인 한 번으로 돌린다 |
