...
- 새 소식과 마감 알림은 매시 작업에서 로그인 한 번과 과제 목록 응답(`include[]=submission`)을 나눠 쓴다. 두 기능의 잠금을 네트워크 전에 한꺼번에 잡아서, 같은 회차에 작업이 겹쳐 들어와도 로그인이 늘지 않는다. 자동 다운로드는 지금처럼 따로 로그인한다. LINUS는 계정당 가장 최근 로그인 하나만 유효해 로그인이 늘면 다른 기기 세션이 더 자주 끊기기 때문이다.
